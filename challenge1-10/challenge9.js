...
// Charlie doesn't like these sessions, since the motive usually repeats.He isn't fond
// of seeing the Eiffel tower 40 times.He tells them that he will only sit during the
// session if they show the same motive at most N times.Luckily, Alice and Bob are able
// to encode the motive as a number.Can you help them to remove numbers such that their
...
function deleteNth(arr, n) {
  let newArray = [];
  arr.forEach((element) => {
    if (countCopies(newArray, element) < n) {
...
deleteNth([3,5,7,3,2,4,6,3,1,4,6,8,54,6,45,23,4,2,4,5],3)
